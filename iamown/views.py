...
class TaskLibraryUpdate(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        if form.instance.task_status == 'Completed':
            ...
            library_task.task_history_log = library_task.task_history_log + f'''>>> Task type: <strong>{form.instance.task_type}</strong> manually updated by <strong>{self.request.user}</strong> on <strong>{get_current_date()}</strong>.<br>
