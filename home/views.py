...
# ####################### Librarian Training #######################
@login_required
def librarian_training(request):
    context = {
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            # If the email is not already subscribed update the new subscription log with entry and change subscription status to Unconfirmed
                ...
                subsciption_outcome_message = f'Bless You for your intention to subscribe. One more step is required to complete your subscription! An email with a link to confirm your email subscription has been sent to {form.instance.email}. If this email does not appear in your inbox within a couple of minutes please check the Spam folder of your email client. Love and Blessings.'
                messages.add_message(
                    self.request,
                    messages.SUCCESS,
                    f'{subsciption_outcome_message}'
